...
def apply_mcdiff(file_path, diff_content):
    ...
    # Parse the GitHub-style diff
    diff_lines = diff_content.splitlines()
    
    # Group diff lines into blocks (chunks of consecutive additions or removals)
    diff_blocks = []
    current_block = {'removals': [], 'additions': []}
    
    for line in diff_lines:
        if line.startswith('-'):
            current_block['removals'].append(line[1:])  # Remove the '-' prefix
        elif line.startswith('+'):
            current_block['additions'].append(line[1:])  # Remove the '+' prefix
        elif line.strip() == '':  # Empty line indicates separation between diff blocks
            if current_block['removals'] or current_block['additions']:
                diff_blocks.append(current_block)
                current_block = {'removals': [], 'additions': []}
    
    # Add the last block if it contains changes
    if current_block['removals'] or current_block['additions']:
        diff_blocks.append(current_block)
    
    # Apply each diff block to the original content
    new_content = '\n'.join(original_lines)
    ...
    for block in diff_blocks:
        ...
        if removals:
            new_content = new_content.replace(removals, additions)
        elif additions:
            # This is just an addition without removal - append it (shouldn't typically happen in standard git diffs)
            new_content += '\n' + additions
